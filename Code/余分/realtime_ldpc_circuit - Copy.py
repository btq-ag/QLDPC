...
class QuantumLDPCCode:
    ...
    def _update_syndrome(self):
        ...
        for i in range(self.n_checks):
            # Count X errors in this parity check
            connected_qubits = np.where(self.parity_matrix[i] == 1)[0]
            x_errors = sum(1 for q in connected_qubits 
                          if self.qubit_states[q] in [2, 4])  # X or Y errors
            self.syndrome[i] = x_errors % 2
...
class LDPCCircuitAnimation:
    ...
    def draw_syndrome(self):
        ...
        for i, syndrome_bit in enumerate(self.ldpc_code.syndrome):
            ...
            rect = Rectangle((0, i), 1, 0.8, facecolor=color, alpha=0.8)
    ...
    def draw_parameters(self):
        ...
        self.ax_params.text(0.5, 0.2, instructions, ha='center', va='center',
    ...
    def update(self, frame):
        ...
        if self.frame_count % 60 == 0:
            ...
            self.fig.suptitle(f"Real-Time Quantum LDPC Circuit Simulator (FPS: {fps:.1f})",
...
if __name__ == "__main__":
    ...
    # Create LDPC code and start simulation
