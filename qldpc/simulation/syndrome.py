...
from qldpc.theme import applyDarkRcParams
...
def create_syndrome_extraction_circuit():
    ...
    for i, y in enumerate(all_y_positions):
        ...
        if y < n_data_qubits:
            ...
            ax.text(
                -0.5, y, f"$|a_{y - n_data_qubits}\\rangle$", ha="right", va="center", fontsize=12
            )
    ...
    for check_idx, (time, check_qubits) in enumerate(zip(gate_times, parity_checks)):
        ...
        h_rect = FancyBboxPatch(
            (time - 0.15, ancilla_qubit - 0.15),
            0.3,
            0.3,
            boxstyle="round,pad=0.02",
            facecolor=lightCmap(0.3),
            edgecolor="#aaaaaa",
        )
        ax.add_patch(h_rect)
        ...
        for data_qubit in check_qubits:
            ...
            target_circle = Circle(
                (time, data_qubit), 0.15, fill=False, edgecolor="#aaaaaa", linewidth=2
            )
            ax.add_patch(target_circle)
            ax.plot(
                [time - 0.1, time + 0.1], [data_qubit, data_qubit], color="#cccccc", linewidth=2
            )
            ax.plot(
                [time, time], [data_qubit - 0.1, data_qubit + 0.1], color="#cccccc", linewidth=2
            )

            # Connection line
            ax.plot(
                [time, time],
                [min(ancilla_qubit, data_qubit) + 0.1, max(ancilla_qubit, data_qubit) - 0.1],
                color="#cccccc",
                linewidth=1,
            )
    ...
    for i in range(n_ancilla_qubits):
        ...
        meas_rect = FancyBboxPatch(
            (meas_time - 0.2, ancilla_qubit - 0.2),
            0.4,
            0.4,
            boxstyle="round,pad=0.02",
            facecolor=divCmap(0.5),
            edgecolor="#aaaaaa",
        )
        ax.add_patch(meas_rect)
        ax.text(
            meas_time, ancilla_qubit, "M", ha="center", va="center", fontsize=10, fontweight="bold"
        )

        # Classical bit output
        ax.plot(
            [meas_time + 0.2, meas_time + 0.8],
    ...
    ax.set_title(
        "Fault-Tolerant Syndrome Extraction Circuit\n"
        + "DiVincenzo-Aliferis Protocol for Quantum LDPC Codes",
        fontsize=16,
    )
    ax.set_aspect("equal")
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "syndrome_extraction_circuit.png"
    )
...
def create_syndrome_error_analysis():
    ...
    for i, strategy in enumerate(strategies):
        ...
        ax1.loglog(p_phys, p_synd, linewidth=2.5, color=colors[i], label=strategy)
    ...
    # Save the plot
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "syndrome_error_analysis.png"
    )
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def create_fault_tolerant_measurement():
    ...
    for idx, (ax, protocol) in enumerate(zip(axes.flatten(), protocols)):
        ...
        for i in range(n_ancilla):
            ...
            if "Flagged" in protocol and i == n_ancilla - 1:
                ax.text(-0.3, y_pos, "$f$", ha="right", va="center", fontsize=11, color="#5dade2")
            else:
        ...
        if protocol == "Standard Measurement":
            ...
            # Measurement
            ax.add_patch(
                Rectangle((5, n_data - 0.2), 0.4, 0.4, facecolor=divCmap(0.5), edgecolor="#aaaaaa")
            )
            ...
            for round_idx in range(rounds + 1):
                ...
                for i in range(n_data // 2):
                    ...
                    ax.add_patch(
                        Circle((t, n_data + round_idx), 0.12, fill=False, edgecolor="#aaaaaa")
                    )

                # Measurement for each ancilla
                ax.add_patch(
                    Rectangle(
                        (t + 0.8, n_data + round_idx - 0.15),
                        0.3,
                        0.3,
                        facecolor=divCmap(0.5),
                        edgecolor="#aaaaaa",
    ...
    # Save the plot
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "fault_tolerant_measurement.png"
    )
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
