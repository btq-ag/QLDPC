...
def create_ghz_fidelity_analysis():
    ...
    for i, C in enumerate(C_values):
        ...
        ax1.plot(
            ...
            linewidth=2.5,
            markersize=6,
            label=f"$C = 10^{{{int(np.log10(C))}}}$",
        )

    # Add threshold line
    ax1.axhline(
        y=0.9, color="red", linestyle="--", linewidth=2, alpha=0.7, label="Target Fidelity (90%)"
    )
    ...
    for i, n in enumerate(n_values):
        ...
        ax2.loglog(C_range, prep_times, linewidth=2.5, color=prep_colors[i], label=f"{n}-qubit GHZ")
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "ghz_fidelity_analysis.png"
    )
...
def create_ghz_preparation_protocol():
    ...
    # Draw cavity
    cavity_rect = Rectangle(
        (0.5, 1), 9, 4, fill=False, edgecolor="#aaaaaa", linewidth=2, linestyle="--"
    )
    ...
    for i in range(1, n_atoms):
        ax.plot(
            [atom_positions[0], atom_positions[i]],
            [3.4, 3.4],
            color="#ff6b6b",
            linewidth=2,
            alpha=0.7,
        ...
        ax.text(
            (atom_positions[0] + atom_positions[i]) / 2,
            3.8,
            "CNOT",
            ha="center",
            fontsize=10,
    ...
    for i, x in enumerate(atom_positions):
        ...
        detector = Rectangle((x - 0.2, 1.5), 0.4, 0.8, color="#999999", alpha=0.6)
        ax.add_patch(detector)
        ax.plot([x, x], [2.6, 2.3], color="#cccccc", linewidth=2)
    ...
    ax.text(
        5,
        4.5,
        r"$|\psi_{GHZ}\rangle = \frac{1}{\sqrt{2}}(|00000\rangle + |11111\rangle)$",
        ha="center",
        va="center",
        fontsize=14,
        fontweight="bold",
    ...
    # Save the plot
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "ghz_preparation_protocol.png"
    )
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def create_ghz_scaling_analysis():
    ...
    # Save the plot
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "ghz_scaling_analysis.png"
    )
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
