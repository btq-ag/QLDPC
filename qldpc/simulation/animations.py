...
def create_ldpc_tanner_graph_animation():
    ...
    def animate(frame):
        ...
        for j, (node, pos) in enumerate(check_positions.items()):
            ...
            square = Rectangle((pos[0] - 0.15, pos[1] - 0.15), 0.3, 0.3, facecolor=color, alpha=0.8)
        ...
        for j in range(m):
            for i in range(n):
                ...
                rect = Rectangle(
                    (i - 0.4, j - 0.4), 0.8, 0.8, facecolor=color, alpha=alpha, edgecolor="#aaaaaa"
                )
                ax2.add_patch(rect)

                if H[j, i] == 1:
                    ax2.text(i, j, "1", ha="center", va="center", fontsize=12, fontweight="bold")
    ...
    anim = animation.FuncAnimation(
        fig, animate, init_func=init, frames=frames, interval=150, blit=False, repeat=True
    )

    # Save animation
    plt.tight_layout()
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "ldpc_tanner_graph_animation.gif"
    )
...
def create_error_correction_process_animation():
    ...
    def animate(frame):
        ...
        for i, pos in enumerate(qubit_positions):
            ...
            ax1.add_patch(circle)
            ax1.text(
                pos, 1, f"$q_{{{i}}}$", ha="center", va="center", fontsize=10, fontweight="bold"
            )
        ...
        ax1.text(
            7,
            -1,
            f"Logical Qubits: $k = {k}$, Physical Qubits: $n = {n}$\n"
        ...
        # Decoding process
        ax3.set_title("LDPC Decoding: Belief Propagation Algorithm", fontsize=14, fontweight="bold")
        ax3.set_xlim(-1, 15)
        ax3.set_ylim(-3, 4)
        ax3.axis("off")
        ...
        if phase >= 3:
            ...
            for i in range(min(8, n)):  # Show subset for clarity
                ...
                if i < 4:
                    ...
                    square = Rectangle(
                        (check_x - 0.25, check_y - 0.25), 0.5, 0.5, facecolor=color, alpha=0.8
                    )
                    ...
                    for j in range(2):
                        ...
                        if var_x >= 0 and var_x <= 14:
                            ...
                            ax3.annotate(
                                "",
                                xy=(check_x, check_y + 0.25),
                                xytext=(var_x, var_y - 0.25),
                                arrowprops=dict(
                                    arrowstyle="->", color=lightCmap(0.3), alpha=0.6, lw=1.5
                                ),
                            )
                            ...
                            ax3.annotate(
                                ...
                                arrowprops=dict(
                                    arrowstyle="->", color=lightCmap(0.7), alpha=0.6, lw=1.5
                                ),
                            )
            ...
            ax3.text(
                7,
                3.5,
                f"Belief Propagation Iteration: {iteration}/4",
                ha="center",
                fontsize=12,
                fontweight="bold",
                bbox=dict(boxstyle="round", facecolor="#1e1e1e", alpha=0.8),
            ...
            if iteration >= 4:
                ax3.text(
                    ...
                    color="#00cc66",
                    fontweight="bold",
                    bbox=dict(boxstyle="round", facecolor="#0a3a1a", alpha=0.3),
                )
    ...
    # Create animation
    anim = animation.FuncAnimation(
        fig, animate, init_func=init, frames=num_frames, interval=200, blit=False, repeat=True
    )

    # Save animation
    plt.tight_layout()
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "ldpc_error_correction_animation.gif"
    )
...
def create_threshold_behavior_animation():
    ...
    def animate(frame):
        ...
        if max_distance_idx > 0:
            ax1.axvline(
                ...
                linewidth=2,
                alpha=0.8,
                label=f"Threshold $p_{{th}} \\approx {threshold}$",
            )
        ...
        ax2.set_title(
            "LDPC Code Scaling\nLinear Distance Achievement", fontsize=14, fontweight="bold"
        )
        ...
        if regime >= 0:
            ...
            ax2.loglog(
                ...
                linewidth=3,
                alpha=0.7,
                label="Classical LDPC: $d \\sim \\sqrt{n}$",
            )
        ...
        if regime >= 1:
            ...
            ax2.loglog(
                ...
                linewidth=3,
                alpha=0.7,
                label="Hypergraph Product: $d \\sim \\sqrt{n \\log n}$",
            )
        ...
        if regime >= 2:
            ...
            ax2.loglog(
                ...
                linewidth=4,
                alpha=0.9,
                label="Quantum Tanner: $d \\sim n$ (Breakthrough!)",
            )
    ...
    plt.tight_layout()
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "ldpc_threshold_behavior_animation.gif"
    )
