...
def create_cavity_cooperativity_analysis():
    ...
    # Plot the main curve
    ax.semilogx(C_values, gate_fidelity, linewidth=3, color=seqCmap(0.8), label="Gate Fidelity")

    # Add threshold line
    ax.axhline(
        y=threshold_fidelity,
        color=seqCmap(0.3),
        linestyle="--",
        linewidth=2,
        label=f"Fault-Tolerant Threshold ({threshold_fidelity})",
    )
    ...
    ax.axvspan(
        C_min, C_max, alpha=0.2, color=seqCmap(0.5), label="Required Range: $C \\sim 10^4 - 10^6$"
    )
    ...
    ax.set_title(
        "Cavity Cooperativity Requirements for Non-Local Gates\n" + "Quantum LDPC Error Correction",
        fontsize=16,
    )
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "cavity_cooperativity.png"
    )
...
def create_trilayer_architecture():
    ...
    for i in range(ancilla_qubits):
        ...
        for j in range(4):
            if start_data + j < data_qubits:
                ax.plot(
                    [data_x[start_data + j], ancilla_x[i]],
    ...
    for i in range(cavity_modes):
        ...
        for j in range(2):
            if start_ancilla + j < ancilla_qubits:
                ax.plot(
                    [ancilla_x[start_ancilla + j], cavity_x[i]],
    ...
    ax.set_title(
        "Tri-Layer Architecture for Quantum LDPC Codes\n"
        + "Cavity-Mediated Non-Local Error Correction",
        fontsize=16,
    )
    ...
    # Save the plot
    save_path = os.path.join(
        os.path.dirname(__file__), "..", "..", "Plots", "trilayer_architecture.png"
    )
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close()
...
def create_error_threshold_analysis():
    ...
    for i, d in enumerate(distances):
        ...
        ax.loglog(p_phys, p_logical, color=colors[i], linewidth=2.5, label=f"Distance d = {d}")
