    ...
    if QISKIT_AVAILABLE:
        try:
            ...
            save_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "Plots", "cavity_cnot_circuit.png"
            )
            ...
            pass  # Fall through to manual drawing
...
def create_manual_circuit_cnot():
    ...
    for gate in gates:
        ...
        if gate_type == "H":
            ...
            # Control dot
            ax.add_patch(plt.Circle((time, control), 0.1, color="#cccccc"))
            # Target circle
            ax.add_patch(
                plt.Circle((time, target), 0.15, fill=False, edgecolor="#aaaaaa", linewidth=2)
            )
            ax.plot([time - 0.1, time + 0.1], [target, target], color="#cccccc", linewidth=2)
            ax.plot([time, time], [target - 0.1, target + 0.1], color="#cccccc", linewidth=2)
            # Connection line
            ax.plot(
                [time, time],
                [min(control, target) + 0.1, max(control, target) - 0.1],
                color="#cccccc",
                linewidth=1,
            )
    ...
    ax.set_title(
        "Cavity-Mediated CNOT Gate Implementation\n" + "Non-Local Quantum Gate via Optical Cavity",
        fontsize=16,
    )
    ax.set_aspect("equal")
    ax.axis("off")
    ...
    # Add explanation
    explanation = (
        "1. Initialize cavity in superposition (H)\n"
...
def create_ghz_preparation_circuit():
    ...
    if QISKIT_AVAILABLE:
        try:
            ...
            # Save the circuit
            save_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "Plots", "ghz_preparation_circuit.png"
            )
            circuit_fig.savefig(save_path, dpi=300, bbox_inches="tight")
            plt.close(circuit_fig)
...
def create_manual_ghz_circuit():
    ...
    for i in range(n_qubits):
        ...
        ax.text(
            -0.3, i, f"$|q_{i}\\rangle$", ha="right", va="center", fontsize=12, fontweight="bold"
        )
    ...
    h_rect = plt.Rectangle(
        (1 - 0.15, 0 - 0.15), 0.3, 0.3, facecolor=lightCmap(0.3), edgecolor="#aaaaaa", linewidth=2
    )
    ax.add_patch(h_rect)
    ...
    for i, time in enumerate(cnot_positions):
        ...
        ax.add_patch(plt.Circle((time, 0), 0.1, color="#cccccc"))

        # Target on other qubits
        ax.add_patch(
            plt.Circle((time, target_qubit), 0.15, fill=False, edgecolor="#aaaaaa", linewidth=2)
        )
        ax.plot(
            [time - 0.1, time + 0.1], [target_qubit, target_qubit], color="#cccccc", linewidth=2
        )
        ax.plot(
            [time, time], [target_qubit - 0.1, target_qubit + 0.1], color="#cccccc", linewidth=2
        )

        # Connection line
        ax.plot([time, time], [0.1, target_qubit - 0.1], color="#cccccc", linewidth=1)
    ...
    ax.set_title(
        "5-Qubit GHZ State Preparation Circuit\n"
        + "Essential Building Block for Quantum LDPC Codes",
        fontsize=16,
    )
    ax.set_aspect("equal")
    ax.axis("off")
...
def create_error_correction_circuit():
    ...
    if QISKIT_AVAILABLE:
        try:
            ...
            # Save the circuit
            save_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "Plots", "error_correction_circuit.png"
            )
            circuit_fig.savefig(save_path, dpi=300, bbox_inches="tight")
            plt.close(circuit_fig)
...
def create_manual_error_correction_circuit():
    ...
    time1 = 2
    ax.add_patch(plt.Circle((time1, 0), 0.1, color="#cccccc"))  # Control on d0
    ax.add_patch(
        plt.Circle((time1, 3), 0.15, fill=False, edgecolor="#aaaaaa", linewidth=2)
    )  # Target on a0
    ax.plot([time1 - 0.1, time1 + 0.1], [3, 3], color="#cccccc", linewidth=2)
    ax.plot([time1, time1], [2.9, 3.1], color="#cccccc", linewidth=2)
    ax.plot([time1, time1], [0.1, 2.9], color="#cccccc", linewidth=1)

    ax.add_patch(plt.Circle((time1 + 0.5, 1), 0.1, color="#cccccc"))  # Control on d1
    ax.add_patch(
        plt.Circle((time1 + 0.5, 3), 0.15, fill=False, edgecolor="#aaaaaa", linewidth=2)
    )  # Target on a0
    ax.plot([time1 + 0.5 - 0.1, time1 + 0.5 + 0.1], [3, 3], color="#cccccc", linewidth=2)
    ax.plot([time1 + 0.5, time1 + 0.5], [2.9, 3.1], color="#cccccc", linewidth=2)
    ax.plot([time1 + 0.5, time1 + 0.5], [1.1, 2.9], color="#cccccc", linewidth=1)

    # Parity check 2: d1 ⊕ d2
    time2 = 4
    ax.add_patch(plt.Circle((time2, 1), 0.1, color="#cccccc"))  # Control on d1
    ax.add_patch(
        plt.Circle((time2, 4), 0.15, fill=False, edgecolor="#aaaaaa", linewidth=2)
    )  # Target on a1
    ax.plot([time2 - 0.1, time2 + 0.1], [4, 4], color="#cccccc", linewidth=2)
    ax.plot([time2, time2], [3.9, 4.1], color="#cccccc", linewidth=2)
    ax.plot([time2, time2], [1.1, 3.9], color="#cccccc", linewidth=1)

    ax.add_patch(plt.Circle((time2 + 0.5, 2), 0.1, color="#cccccc"))  # Control on d2
    ax.add_patch(
        plt.Circle((time2 + 0.5, 4), 0.15, fill=False, edgecolor="#aaaaaa", linewidth=2)
    )  # Target on a1
    ax.plot([time2 + 0.5 - 0.1, time2 + 0.5 + 0.1], [4, 4], color="#cccccc", linewidth=2)
    ax.plot([time2 + 0.5, time2 + 0.5], [3.9, 4.1], color="#cccccc", linewidth=2)
    ax.plot([time2 + 0.5, time2 + 0.5], [2.1, 3.9], color="#cccccc", linewidth=1)

    # Measurements
    meas_time = 5.5
    for i in range(n_ancilla_qubits):
        y_pos = n_data_qubits + i
        meas_rect = plt.Rectangle(
            (meas_time - 0.15, y_pos - 0.15),
            0.3,
            0.3,
            facecolor=divCmap(0.5),
            edgecolor="#aaaaaa",
